...
serialized and stored in memory.  When ["save"](#save) is called, the transaction cache will be written to 
the file specfied by ["file"](#file) in JSON format in the order they were made.  
...
Instead, do the following (as seen in t/030\_basic\_authentication.t):
    ...
    $mock->get_ok('/')->status_is(200)->text_is( ... );
