...
serialized and stored in memory.  When L</save> is called, the transaction cache will be written to 
the file specfied by L</file> in JSON format in the order they were made.  
...
=head2 Automatic save

Currently, automatic save when the object goes out of scope does not work.
