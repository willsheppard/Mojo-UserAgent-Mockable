...
my $url = Mojo::URL->new(q{/integers})->query(
    num    => $COUNT,
    min    => $MIN,
    max    => $MAX,
    col    => $COLS,
...
SKIP: {
    ...
    subtest 'Reverse order' => sub {
        ...
        {
            my $mock =
                Mojo::UserAgent::Mockable->new( mode => 'playback', file => $output_file, unrecognized => 'null' );
            $mock->server->app($app);
        ...
        {
            my $mock =
                Mojo::UserAgent::Mockable->new( mode => 'playback', file => $output_file, unrecognized => 'exception' );
            $mock->server->app($app);
        ...
        {
            my $mock =
                Mojo::UserAgent::Mockable->new( mode => 'playback', file => $output_file, unrecognized => 'fallback' );
            $mock->server->app($app);
            for ( 0 .. $#transactions ) {
                my $index       = $#transactions - $_;
                my $transaction = $transactions[$index];
